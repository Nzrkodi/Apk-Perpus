...
class Peminjaman extends CI_Controller {
	...
	{	
		...
		if($this->session->userdata('level') == 'Siswa'){
			$this->data['pinjam'] = $this->db->query("SELECT DISTINCT `pinjam_id`, `anggota_id`, 
				`jmlh_buku`, `kelas`, `status`, `tgl_pinjam`, `lama_pinjam`, `tgl_balik`, `tgl_kembali` 
				FROM pinjam_kelas WHERE status = 'Dipinjam' 
				AND anggota_id = ? ORDER BY pinjam_id DESC", 
				array($this->session->userdata('anggota_id')));
		}else{
			$this->data['pinjam'] = $this->db->query("SELECT DISTINCT `pinjam_id`, `anggota_id`, 
				`jmlh_buku`, `kelas`, `status`, `tgl_pinjam`, `lama_pinjam`, `tgl_balik`, `tgl_kembali` 
				FROM pinjam_kelas WHERE status = 'Dipinjam' ORDER BY pinjam_id DESC");
		}
		
		$this->load->view('sidebar_view',$this->data);
        $this->load->view('header_view',$this->data);
		$this->load->view('pinjam_kelas/pinjam_view',$this->data);
	...
	{	
		...
		if($this->session->userdata('level') == 'Siswa'){
			$this->data['pinjam'] = $this->db->query("SELECT DISTINCT `pinjam_id`, `anggota_id`, 
				`jmlh_buku`, `kelas`, `status`, `tgl_pinjam`, `lama_pinjam`, `tgl_balik`, `tgl_kembali` 
				FROM pinjam_kelas WHERE anggota_id = ? AND status = 'Di Kembalikan' 
				ORDER BY id_kelas DESC",array($this->session->userdata('anggota_id')));
		}else{
			$this->data['pinjam'] = $this->db->query("SELECT DISTINCT `pinjam_id`, `anggota_id`, 
				`jmlh_buku`, `kelas`, `status`, `tgl_pinjam`, `lama_pinjam`, `tgl_balik`, `tgl_kembali` 
				FROM pinjam_kelas WHERE status = 'Di Kembalikan' ORDER BY id_kelas DESC");
		}
		
		$this->load->view('sidebar_view',$this->data);
        $this->load->view('header_view',$this->data);
		$this->load->view('kembali_kelas/home',$this->data);
		$this->load->view('footer_view',$this->data);
	}
	...
	{	
		...
		$this->data['nop'] = $this->M_Admin->buat_kode('pinjam_kelas','PJ','id_kelas','ORDER BY id_kelas DESC LIMIT 1'); 
		$this->data['idbo'] = $this->session->userdata('ses_id');
		...
		$this->load->view('sidebar_view',$this->data);
        $this->load->view('header_view',$this->data);
		$this->load->view('pinjam_kelas/tambah_view',$this->data);
		$this->load->view('footer_view',$this->data);
	}
	...
	public function detailpinjam()
	{
		...
		if($this->session->userdata('level') == 'Siswa'){
			$count = $this->db->get_where('pinjam_kelas',[
				'pinjam_id' => $id, 
			...
			if($count > 0)
			{
				...
				`anggota_id`, `jmlh_buku`, `kelas`, `status`, 
				`tgl_pinjam`, `lama_pinjam`, 
				`tgl_balik`, `tgl_kembali` 
				FROM pinjam_kelas WHERE pinjam_id = ? 
				AND anggota_id =?", 
				array($id,$this->session->userdata('anggota_id')))->row();
			}else{
				echo '<script>alert("DETAIL TIDAK DITEMUKAN");window.location="'.base_url('peminjaman').'"</script>';
			}
		}else{
			$count = $this->M_Admin->CountTableId('pinjam_kelas','pinjam_id',$id);
			if($count > 0)
			{
				$this->data['pinjam'] = $this->db->query("SELECT DISTINCT `pinjam_id`, 
				`anggota_id`, `jmlh_buku`, `kelas`, `status`, 
				`tgl_pinjam`, `lama_pinjam`, 
				`tgl_balik`, `tgl_kembali` 
				FROM pinjam_kelas WHERE pinjam_id = '$id'")->row();
			}else{
				echo '<script>alert("DETAIL TIDAK DITEMUKAN");window.location="'.base_url('peminjaman').'"</script>';
			}
		}
		...
		$this->load->view('sidebar_view',$this->data);
        $this->load->view('header_view',$this->data);
		$this->load->view('pinjam_kelas/detail',$this->data);
		$this->load->view('footer_view',$this->data);
	}
	...
	public function kembalipinjam()
	{
		...
		$this->load->view('sidebar_view',$this->data);
        $this->load->view('header_view',$this->data);
		$this->load->view('pinjam_kelas/kembali',$this->data);
		$this->load->view('footer_view',$this->data);
	}
	...
	public function prosespinjam()
	{
		...
		if(!empty($post['tambah']))
		{
			...
			foreach($hasil_cart as $isi)
			{
				$data[] = array(
					...
					'jmlh_buku' => htmlentities($post['jmlh_buku']),
					'tgl_balik'  => $tgl2, 
					'tgl_kembali'  => '0',
				);
			}
			...
			if($total_array != 0)
			{
				$this->db->insert_batch('pinjam_kelas',$data);
		...
		{
			$this->M_Admin->delete_table('pinjam_kelas','pinjam_id',$this->input->get('pinjam_id'));
			$this->M_Admin->delete_table('tbl_denda','pinjam_id',$this->input->get('pinjam_id'));

			$this->session->set_flashdata('pesan','<div id="notifikasi"><div class="alert alert-warning">
			<p>  Hapus peminjaman Pinjam Buku Sukses !</p>
			</div></div>');
			redirect(base_url('peminjaman')); 
		}
		...
		if($this->input->get('kembali'))
		{
			...
			$pinjam = $this->db->query("SELECT  * FROM pinjam_kelas WHERE pinjam_id = '$id'");

			foreach($pinjam->result_array() as $isi){
				$pinjam_id = $isi['pinjam_id'];
				$denda = $this->db->query("SELECT * FROM tbl_denda WHERE pinjam_id = '$pinjam_id'");
				$jml = $this->db->query("SELECT * FROM pinjam_kelas WHERE pinjam_id = '$pinjam_id'")->num_rows();			
				if($denda->num_rows() > 0){
					$s = $denda->row();
					echo $s->denda;
				}else{
			...
			{	
				...
				$this->db->update('pinjam_kelas',$data);
			}
	...
	public function denda()
	{
		...
		if(!empty($this->input->get('id'))){
			...
			{			
				...
				echo '<script>alert("KATEGORI TIDAK DITEMUKAN");window.location="'.base_url('peminjaman/denda').'"</script>';
			}
		}
	...
	public function dendaproses()
	{
		if(!empty($this->input->post('tambah')))
		{
			...
			redirect(base_url('peminjaman/denda')); 
		}
